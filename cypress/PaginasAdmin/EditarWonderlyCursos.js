...
export class EditarWonderlyCursos {
  ...
  click_editarClase() {
    cy.xpath(
      "/html/body/div[2]/div[2]/div/div/div/main/article/div[2]/table/tbody/tr[1]/td[6]/a[1]"
    ).click();
  }
  ...
  buscar_curso(nombreCurso, accion) {
    ...
    for (var i = 1; i <= 20; i++) {
      ...
      cy.xpath(
        ...
        .then((text) => {
          let curso = text.trim();
          cy.log("NOMBRE CURSO", nombreCurso);
          if (curso == nombreCurso) {
            existeCurso = contador;
          }
          cy.log("EXISTE CURSO::: ", existeCurso);
          if (existeCurso != false && contador === 20) {
            cy.xpath(
              "/html/body/div[2]/div[2]/div/div/div/main/article/div[3]/table/tbody/tr[" +
                existeCurso +
                "]/td[6]/a[" +
